...
namespace LockscreenWallpaperRetriever
{
    public class ImageFinder
    {
        ...
        public async Task<List<FileInfo>> FindAsync(bool isGetRecent = true)
        {
            ...
            await FilterFilesByImageSizeAsync();

            return _currentQualifiedFiles;
        }

        public List<FileInfo> Find(bool isGetLatest = true)
        {
            GetAllFilesInAssetFolder();
            FilterFilesBySize();
            if (isGetLatest)
        ...
        private async Task FilterFilesByImageSizeAsync()
        {
            _imageSizeCheckBgWorker = new BackgroundWorker();
            _imageSizeCheckBgWorker.DoWork += FilterFilesByImageSize;
            _imageSizeCheckBgWorker.RunWorkerAsync();

            while (_imageSizeCheckBgWorker.IsBusy)
            {
                await Task.Delay(10);
            }
        }
